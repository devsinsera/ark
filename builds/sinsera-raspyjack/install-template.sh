...
if [[ -f "$BOOT_DIR/raspyjack-src.tar.gz" ]]; then
  ark_log "✓ raspyjack-src.tar.gz present on boot partition: $(stat --printf='%s' "$BOOT_DIR/raspyjack-src.tar.gz") bytes"
else
  ark_log "WARN: raspyjack-src.tar.gz NOT found on boot partition — first boot will fall back to git clone"
fi
...
if [ -f "$BOOT/raspyjack-src.tar.gz" ]; then
  echo "[sinsera-raspyjack] extracting bundled source tarball"
  tar -xzf "$BOOT/raspyjack-src.tar.gz" -C /opt/raspyjack
  # Optionally move the tarball off boot to free space; keep a copy
  # in /var/lib for reinstall.
  mkdir -p /var/lib/raspyjack
  mv "$BOOT/raspyjack-src.tar.gz" /var/lib/raspyjack/source.tar.gz
else
