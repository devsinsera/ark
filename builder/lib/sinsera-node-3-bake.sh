#!/bin/bash
# sinsera-node-3-bake.sh — Pi ZERO 2 W → 75" TV Vigil camera wall. Renders the 4
# Eufy cams as a 2x2 grid on the HDMI framebuffer (Pygame/kmsdrm, NO browser — the
# Zero 2 W can't run WPE/cog). LAN-local (reaches the bridge at 192.168.4.163:8091
# over wifi). LCD HAT off. Lean: no Claude/USB-builder/WireGuard.
# Output → Dev-Sinsera/Builds/sinsera-node-3-zero2w.img
set -euo pipefail
...
echo "[node3-bake] deliver → $BUILDS/sinsera-node-3-zero2w.img"
mkdir -p "$BUILDS"
mv -f "$OUT_IMG" "$BUILDS/sinsera-node-3-zero2w.img"
echo ""; echo "[node3-bake] DONE:"
echo "  $BUILDS/sinsera-node-3-zero2w.img"
echo "  → Vigil camera wall on HDMI (framebuffer, 1080p) · LCD HAT off · wifi · ssh peta@sinsera-node-3.local"
