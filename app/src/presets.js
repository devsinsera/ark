...
// ── OS presets ─────────────────────────────────────────────────────
// `base_image_glob` matches files in Os/ that this preset can use as
// a starting point. The /api/images Hub endpoint lists what's
// actually present on disk; this catalogue describes what's
// *available* (whether or not the operator has downloaded it).
export const OS_PRESETS = {
  'dietpi-rpi5': {
    id: 'dietpi-rpi5',
    label: 'DietPi (RPi 5 · ARMv8)',
    description: 'Minimal Debian-based, sub-1 GB. Verified base for Ark; the SinseraCore image was built from this.',
    base_image_glob: 'DietPi_RPi5-ARMv8-*.img*',
    download_url:    'https://dietpi.com/downloads/images/DietPi_RPi5-ARMv8-Trixie.img.xz',
    package_manager: 'apt',
    default_user:    'root',
    has_systemd:     true,
    suggested_hardware: ['pi-5-4gb', 'pi-5-8gb', 'pi-5-16gb'],
  },
  'dietpi-rpi2': {
    id: 'dietpi-rpi2',
    label: 'DietPi (RPi 2 / Zero 2 W · ARMv7)',
    description: 'DietPi for 32-bit ARMv7 Pis. Use for Pi 2, Pi 3 (running in armv7 mode), and Pi Zero 2 W.',
    base_image_glob: 'DietPi_RPi2-ARMv7-*.img*',
    download_url:    'https://dietpi.com/downloads/images/DietPi_RPi2-ARMv7-Trixie.img.xz',
    package_manager: 'apt',
    default_user:    'root',
    has_systemd:     true,
    suggested_hardware: ['pi-zero-2w', 'pi-3b-plus'],
  },
  'dietpi-rpi1': {
    id: 'dietpi-rpi1',
    label: 'DietPi (RPi 1 / Zero · ARMv6)',
    description: 'DietPi for ARMv6 Pis. Pi 1, Pi Zero, Pi Zero W.',
    base_image_glob: 'DietPi_RPi1-ARMv6-*.img*',
    download_url:    'https://dietpi.com/downloads/images/DietPi_RPi1-ARMv6-Trixie.img.xz',
    package_manager: 'apt',
    default_user:    'root',
    has_systemd:     true,
    suggested_hardware: ['pi-zero-2w'],   // Zero 2 W can run armv6 too
  },
  'pi-os-lite': {
    id: 'pi-os-lite',
    label: 'Pi OS Lite (64-bit)',
    description: 'Official Raspberry Pi OS, headless. Most up-to-date kernel + firmware.',
    base_image_glob: 'raspios_lite*arm64*.img*',
  ...
  'pi-os-desktop': {
    ...
    description: 'Official Pi OS with LXDE pre-installed. Best base for kiosk / signage builds — chromium and X are already there.',
    base_image_glob: 'raspios_arm64*.img*',    // note: official desktop image filename is just raspios_arm64
    download_url:    'https://downloads.raspberrypi.org/raspios_arm64_latest',
    package_manager: 'apt',
    default_user:    'pi',
    has_systemd:     true,
  ...
  'ubuntu-server': {
    ...
    description: 'Server-grade Ubuntu with long support cycle. Heavier than DietPi but well-known toolchain.',
    base_image_glob: 'ubuntu*server*arm64*.img*',
    download_url:    'https://cdimage.ubuntu.com/releases/24.04/release/ubuntu-24.04.4-preinstalled-server-arm64+raspi.img.xz',
    package_manager: 'apt',
    default_user:    'ubuntu',
    has_systemd:     true,
    suggested_hardware: ['pi-4-4gb', 'pi-4-8gb', 'pi-5-4gb', 'pi-5-8gb', 'pi-5-16gb'],
